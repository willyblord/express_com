...
class CheckoutController extends Controller
{
    ...
    // convert amount shipping fee
    public function shipping_fee(Request $request)
    {
        $total = $request->shipping + $request->total;
        
        return [
            single_price($request->shipping),
            single_price($total),
            convert_price($total)
        ];
    }
        ...
        if ($request->payment_option != null) {
            ...
            if($request->session()->get('order_id') != null){
                ...
                elseif ($request->payment_option == 'wallet') {
                    ...
                    if($request->payment_option == 'afripay') {
                        return view('frontend.payment.afripay', [
                        	'code' => $order->code,
    ...
    public function get_shipping_info(Request $request)
    {
        if(Session::has('cart') && count(Session::get('cart')) > 0){
            ...
            return view('frontend.shipping_info', compact('categories', 'info_button'));
        }
    ...
    public function store_delivery_info(Request $request)
    {
        if(Session::has('cart') && count(Session::get('cart')) > 0){
            ...
            if($cart[0]['shipping_type'] == 'home_delivery') {
                $request->validate([
                    'shipping_localtion' => 'required'
                ]);
            }
            ...
            $data['name'] = Session::get('shipping_info')['name'];
            $data['email'] = Session::get('shipping_info')['email'];
            $data['address'] = Session::get('shipping_info')['address'];
            $data['country'] = Session::get('shipping_info')['country'];
            $data['city'] = Session::get('shipping_info')['city'];
            $data['postal_code'] = Session::get('shipping_info')['postal_code'];
            $data['phone'] = Session::get('shipping_info')['phone'];
            $data['checkout_type'] = Session::get('shipping_info')['checkout_type'];
            $data['shipping_fee'] = (int) $request->shipping_localtion;
            $data['shipping_desc'] = $request->ship_desc;

            $shipping_info = $data;

            $request->session()->put('shipping_info', $shipping_info);

            $total = $subtotal + $tax + (int) $request->shipping_localtion;
            ...
            return view('frontend.payment_select', compact('total', 'ship', 'payment_button'));
        }
        else {
    ...
    public function get_payment_info(Request $request)
    {
        ...
        $total = $subtotal + $tax + Session::get('shipping_info')['shipping_fee'];
