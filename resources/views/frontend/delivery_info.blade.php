                ...
                <form class="form-default" data-toggle="validator" action="{{ route('checkout.store_delivery_info') }}" role="form" method="POST">
                    <div class="row cols-xs-space cols-sm-space cols-md-space">
                        <div class="col-xl-8">
                            ...
                            <div class="card mb-3 mt-1" id="ShippingBox">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label class="control-label">{{__('Shipping Location')}}</label>
                                                ...
                                                <select class="form-control selectpicker {{ $errors->has('shipping_localtion') ? ' is-invalid' : '' }}" name="shipping_localtion" onchange="updateShipFee()" id="ShipAddress">
                                                    <option value="">{{__('All Locations')}}</option>
                                                    @foreach(deliveryAddress() as $address => $amount)
                                                        <option value="{{ ($total >= config('app.shipping.free')) ? 0 : $amount }}">{{ $address }}</option>
                                                    @endforeach
                                                </select>

                                                @if ($errors->has('shipping_localtion'))
                                                    <span class="invalid-feedback" role="alert">
                                                        <strong>{{ $errors->first('shipping_localtion') }}</strong>
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label class="control-label">{{__('Shipping Description')}}</label>
                                                <textarea name="ship_desc" class="form-control" style="resize: none;" autocomplete="off"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script type="text/javascript">
        ...
        function show_pickup_point(el) {
            ...
            if(value == 'home_delivery'){
                $('#ShippingBox').removeClass('display_none');
                ...
                var shipping = 0;
                var total = document.getElementById('BeforeTotalAmount').value;

                document.getElementById('ShipFee').innerHTML = 'loading...';
                document.getElementById('TotalAmount').innerHTML = 'loading...';
                document.getElementById('TotalCodeAmount').innerHTML = 'loading...';
                document.getElementById('CtnPayment').disabled = true;

                $(target).removeClass('d-none');
                $('#ShippingBox').addClass('display_none');

                $.ajax({
                    url: '{{ route("checkout.shipping_fee") }}?shipping=' + shipping + '&total=' + total,
                    method: 'GET',
                    success: function(data) {
                        document.getElementById('ShipFee').innerHTML = data[0];
                        document.getElementById('TotalAmount').innerHTML = data[1];
                        document.getElementById('TotalCodeAmount').innerHTML = data[2];
                        document.getElementById('CtnPayment').disabled = false;
                    }
                });
            }
        }
        ...
        function updateShipFee() {
            ...
            // update ship fee on change
            //var shipFee = parseFloat(document.getElementById('ShipAddress').value);
            //document.getElementById('ShipFee').innerHTML = shipFee;

            // update total on change adding ship fee
            //var total = parseFloat(document.getElementById('BeforeTotalAmount').value);
            //var totalAmount = shipFee + total;
